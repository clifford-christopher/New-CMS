...
backend/app/
├── services/
│   ├── validation_service.py          # Validation logic for all 3 types (NEW)
│   └── configuration_service.py       # Update with validation calls (MODIFY)
├── routers/
│   └── configuration.py               # Add validation endpoint (MODIFY)
└── models/
    └── validation.py                  # Validation result models (NEW)
```
...
├── components/
│   └── config/
│       ├── ValidationModal.tsx        # Validation results modal (NEW)
│       ├── PublishButton.tsx          # Publish with validation (NEW)
│       └── ConfigurationWorkspace.tsx # Add publish button (MODIFY)
└── types/
    └── validation.ts                  # TypeScript validation types (NEW)
```
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>
          {validation.all_passed ? '✅ Validation Passed' : '❌ Validation Failed'}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
6. **Grouped Display**: ValidationModal groups checks by category (Shared → Paid → Unpaid → Crawler) for clarity.

7. **Publish Flow**: Publish button → Validation → ValidationModal → Confirm Publish (Story 5.2).
...
8. All checks displayed with appropriate icons (✓, ⚠, ✗)
