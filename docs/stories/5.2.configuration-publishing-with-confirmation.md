...
2. Confirmation modal displays:
   ...
   - All 3 prompt types with truncated previews and character counts:
     - 💰 Paid prompt (truncated, expandable)
     - 🆓 Unpaid prompt (truncated, expandable)
     - 🕷️ Crawler prompt (truncated, expandable)
...
backend/app/
├── routers/
│   └── configuration.py               # Add publish endpoint (MODIFY)
├── services/
│   ├── publish_service.py             # Publish logic (NEW)
│   ├── diff_service.py                # Configuration diff algorithm (NEW)
│   └── audit_service.py               # Create audit log (NEW)
└── models/
...
├── components/
│   └── config/
│       ├── PublishConfirmationModal.tsx   # Confirmation modal (NEW)
│       ├── PromptDiffView.tsx             # Diff display (NEW)
│       ├── PublishButton.tsx              # Update to show confirmation (MODIFY)
│       └── ValidationModal.tsx            # Update to trigger confirmation (MODIFY)
├── app/
│   └── config/
│       └── [triggerId]/
│           └── published/
│               └── page.tsx               # Published config view (NEW)
└── types/
    └── publish.ts                         # TypeScript publish types (NEW)
```
  ...
  return (
    <Modal show={show} onHide={onHide} size="xl">
      <Modal.Header closeButton>
        <Modal.Title>
          {diff?.is_first_publish ? '📢 Publish Configuration' : '📝 Update Configuration'}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
        ...
        <div className="mb-4">
          ...
          <Row className="mt-2">
            ...
            <Col md={4}>
              ...
              <small className="text-muted">{config.section_order.join(' → ')}</small>
            </Col>
            <Col md={4}>
        ...
        <Alert variant="warning" className="mt-4">
          <strong>⚠️ Important:</strong> This configuration will be immediately available to the production news generation system for all 3 prompt types (paid, unpaid, crawler).
        </Alert>
      </Modal.Body>
      <Modal.Footer>
...
export default function PublishButton({ triggerId, triggerName, onPublishSuccess }: PublishButtonProps) {
  ...
  const handleFinalPublish = async () => {
    try {
      ...
      setSuccessMessage(
        `✅ Published successfully with 3 prompt types as Version ${data.version}`
      );
...
1. **3-Step Publish Flow**: Validation → Confirmation → Publish. User sees exactly what will be published before final confirmation.
...
describe('PublishConfirmationModal Component', () => {
  ...
  test('displays all 3 prompt type previews', () => {
    ...
    expect(screen.getByText(/🆓 Unpaid Prompt/)).toBeInTheDocument();
    expect(screen.getByText(/🕷️ Crawler Prompt/)).toBeInTheDocument();
  });
...
```
Story 5.1 (Validation) → Story 5.2 (Publish with isActive=true) → Story 4.3 (Check isActive flag)
                                                                        ↓
